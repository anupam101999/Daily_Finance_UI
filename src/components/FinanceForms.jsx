...
export function AssetForm({ form, setForm, onSubmit, busy, editing, onCancel, exchangeOptions }) {
  return (
    <form className="asset-form" onSubmit={onSubmit}>
      ...
      <input value={form.averagePrice} onChange={(event) => setForm({ ...form, averagePrice: event.target.value })} placeholder="Buy price per unit" type="number" min="0" step="0.00001" required />
      <input value={form.charges} onChange={(event) => setForm({ ...form, charges: event.target.value })} placeholder="Brokerage / charges" type="number" min="0" step="0.000001" />
...
export function SaleForm({ form, setForm, onSubmit, onCancel, busy, maxQuantity }) {
  return (
    <form className="asset-form" onSubmit={onSubmit}>
      ...
      <input value={form.sellPrice} onChange={(event) => setForm({ ...form, sellPrice: event.target.value })} placeholder="Sell price per unit" type="number" min="0" step="0.00001" required />
      <input value={form.charges} onChange={(event) => setForm({ ...form, charges: event.target.value })} placeholder="Brokerage / charges" type="number" min="0" step="0.000001" />
      <textarea value={form.notes} onChange={(event) => setForm({ ...form, notes: event.target.value })} placeholder="Notes" />
      <div className="button-row">
...
export function DividendForm({ form, setForm, assets, onSubmit, onCancel, busy }) {
  ...
  return (
    <form className="asset-form" onSubmit={onSubmit}>
      ...
      <input value={form.amount} onChange={(event) => setForm({ ...form, amount: event.target.value })} placeholder="Dividend amount received" type="number" min="0" step="0.000001" required />
      <textarea value={form.notes} onChange={(event) => setForm({ ...form, notes: event.target.value })} placeholder="Notes" />
...
export function TransactionForm({ form, setForm, assets, onSubmit, onCancel, busy }) {
  return (
    <form className="asset-form transaction-form" onSubmit={onSubmit}>
      ...
      <Field label="Price or amount"><input value={form.price} onChange={(event) => setForm({ ...form, price: event.target.value })} type="number" min="0" step="0.000001" required /></Field>
      <Field label="Brokerage / charges"><input value={form.charges} onChange={(event) => setForm({ ...form, charges: event.target.value })} type="number" min="0" step="0.000001" /></Field>
